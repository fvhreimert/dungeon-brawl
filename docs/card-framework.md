...
- Passive pacing is centralized through `getPassivePointsForCard`. The hook recalculates `PlayerStats.passivePointsPerTurn` whenever inventory changes so the scoreboard can render a single arrow + value indicating the net passive delta for the next turn, while the detailed tracking (total passive gained, question losses, card losses, etc.) stays tucked inside `PlayerStats`.
...
- Hover tooltips render through a portal anchored to the hovered card’s bounding rect, float above the modal, show the base `description`, and appear only after ~450ms without scrolling. Any scroll/resize/wheel event immediately hides the tooltip so it never clips or duplicates.
 - While passive points-per-turn is always tracked in `PlayerStats`, the scoreboard only renders the aggregate `passivePointsPerTurn` value next to the score when it is non-zero, using the pixel arrow assets to show up/down direction and a single colored value.
...
3. If the card grants passive resources, expose `passivePointsPerTurn` so `calculatePassivePointsPerTurn` can update the passive-meter badge next to the player score.
