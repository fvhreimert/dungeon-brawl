...
- `CARD_CONFIG` also exposes `targetSelectMode`, which the UI uses to decide which player selector to render when a card needs a victim: `'standard'` shows the blood-sacrifice style, `'neutral'` locks to players with cards (e.g., Thieving Rat), `'fel'` renders the Soul Burst/green theme, `'puppet'` opens the Puppet Master category selection flow, `'roulette'` opens the self-targeting gambling modal, `'price_cracker'` opens the Christmas cracker mini-game modal, and `'none'` activates immediately without any target selection. Keep the catalog in sync with any new selection rules you add.
...
- `calculatePassiveDeltaForPlayer(player, players)` re-uses every inventory card's `getPassiveDelta` to compute the total passive delta, plus incoming effects from other players' cards (e.g., Beggar stealing from opponents). The scoreboard reads that value directly so the arrow/badge updates instantly when scores, cards, or state change.
